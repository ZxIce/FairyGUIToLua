...
function genCode(handler)
    local Publish = handler.project:GetSettings("Publish")
...
---@type CS.FairyEditor.PublishHandler.MemberInfo
            ...
            if memberInfo.type == "CS.FairyGUI.GButton" then
                _classFieldInstatiation = _classFieldInstatiation .."\n\t" .. "UIHelper.BindOnClickEvent(".. "self." .. memberInfo.name .. ",function() " .. memberInfo.name .. "OnClickCallBack(self) end)"
            end
        end
        ...
        local _callBackText = ""
        for j = 0, memberCnt - 1 do
            _callBackText = _callBackText .. WriteOnClickFunc(members[j])
        end
        _classTemplateTxt = string.gsub(_classTemplateTxt, "$callBack", _callBackText)
...
function WriteOnClickFunc(_memberInfo)
    local str = ""
    if  _memberInfo.type == "CS.FairyGUI.GButton" then
